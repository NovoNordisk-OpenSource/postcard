...
ate <- rctglm(
  ...
  data = dat_treat,
  family = "gaussian", # Default value
  estimand_fun = "ate" # Default value
)
```
...
-   A list of learners to fit, among which the best learner in regards to RMSE is found
-   Number folds in cross validation when testing models
...
As already described, this process fits a discrete super learner to the historical data with the code
```{r, eval = FALSE}
fit_best_learner(
  preproc = list(mod = Y ~ .),
  data = dat_notreat,
...
and uses the resulting model to find prognostic scores that are then adjusted for in the analysis run by `rctglm()`.
...
```{r}
ancova <- glm(Y ~ X, data = dat_notreat)
preds_anc <- predict(ancova, dat_notreat)
lrnr <- fit_best_learner(list(mod = Y ~ X), data = dat_notreat)
...
var_bound_ancova <- variance_ancova(Y ~ X, data = dat_notreat)
var_bound_prog <- variance_ancova(Y ~ X + .pred, data = dat_notreat)
```
