test_that("`rctglm` snapshot tests", {
  ...
  dat_gaus <- sim_rct_data()
  dat_pois <- sim_rct_data(family = poisson())

  ate_with_cv <- fit_rctglm_ate(dat_gaus, cv_variance = TRUE)
  expect_s3_class(ate_with_cv, "rctglm")
  expect_snapshot(estimand(ate_with_cv))

  ate_wo_cv <- fit_rctglm_ate(dat_gaus, cv_variance = FALSE)
  expect_snapshot(estimand(ate_wo_cv))

  rr_with_cv <- fit_rctglm_ate(dat_pois, family = poisson(), cv_variance = TRUE)
  expect_snapshot(estimand(rr_with_cv))

  rr_wo_cv <- fit_rctglm_ate(dat_pois, family = poisson(), cv_variance = FALSE)
...
test_that("Different `cv_variance_folds` produces same estimate but different estimated SE", {
  ...
  dat_gaus <- sim_rct_data()
  ate_with_cv <- fit_rctglm_ate(dat_gaus, cv_variance = TRUE, cv_variance_folds = 2)
  ate_with_cv_difffolds <- fit_rctglm_ate(dat_gaus, cv_variance = TRUE, cv_variance_folds = 10)
  ate_wo_cv <- fit_rctglm_ate(dat_gaus, cv_variance = FALSE)
...
test_that("`estimand_fun` argument can be specified as function or character", {
  ...
  nonsense <- fit_rctglm_ate(dat_gaus, estimand_fun = nonsense_estimand_fun)
  expect_equal(nonsense$estimand_funs$f, nonsense_estimand_fun)

  # Error when giving character that is not among the defaults
  expect_error(fit_rctglm_ate(dat_gaus, estimand_fun = "test"),
